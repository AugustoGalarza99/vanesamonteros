...
const EstadoPeluquero = () => {
    ...
    const handleBuscarTurno = async () => {
        try {
            ...
            const q = query(reservasRef, where('dni', '==', dni), where('status', '==', 'Pendiente'));
            const querySnapshot = await getDocs(q);
    
            if (!querySnapshot.empty) {
                // Obtener el primer turno pendiente del cliente
                const turnoClienteDoc = querySnapshot.docs[0];
                const turnoCliente = { id: turnoClienteDoc.id, ...turnoClienteDoc.data() };
                ...
                const turnosPrevios = querySnapshot.docs
                    .map(doc => ({ id: doc.id, ...doc.data() })) // Mapear todos los documentos a objetos
                    .filter(reserva =>
